...
int main() {
  ...
  // filesystem::recursive_directory_iterator("./raspi-cpp/testimages")){
  while (true) {
    float correction = 0;
    ...
    // cv::Mat mask;
    // cv::inRange(frame, cv::Scalar(100, 100, 100), cv::Scalar(255, 255, 255),
    // mask); entropy.setTo(cv::Scalar(0,0,0), mask);

    vector<cv::Vec3f> circles;
    cv::HoughCircles(entropy2, circles, cv::HOUGH_GRADIENT, 1, 10, 54, 26, 15,
                     150);
    ...
    if (circles.size() > 0) {
      ...
      for (cluster c : clusters) {
        ...
        for (cv::Point2i p : cpoints) {
          ...
          available_radiuses.push_back(
              radiuses[distance(centers.begin(), match)]);
        }
        // cluster_radiuses.push_back(*(available_radiuses.begin(),
        // available_radiuses.end()));
      ...
      for (size_t i = 0; i < clusters.size(); i++) {
        ...
        for (int j = -2; j < 3; j++) {
          ...
          const int x =
              get<0>(clusters[i]).x + copysign(radiuses[i], j) + diff * j;
          if (!(frame.cols > x and x > 0)) {
            continue;
          };
          if (frame.at<cv::Scalar>(int(x), int(y))[0] > 200) {
            cv::circle(frame, cv::Point2i(x, y), 3, cv::Scalar(0, 0, 255), -1,
                       8, 0);
            valid = 0;
            break;
          }
    ...
    // cv::Mat mask;
    // inRange(frame, cv::Scalar(210, 210, 210), cv::Scalar(255, 255, 255),
    // mask);
    ...
    if (clusters.size() > 0) {
      cv::line(frame, cv::Point2i(int(frame.cols / 2), frame.rows),
               cv::Point2i(get<0>(clusters[0]).x, int(frame.rows / 2)),
               cv::Scalar(0, 0, 255), 2);
      correction = degrees(asin((get<0>(clusters[0]).x - frame.cols / 2) /
                                (frame.rows / 2))) /
                   90 * (FOV / 2);
    }
    cv::putText(frame, to_string(correction) + "d", cv::Point2i(10, frame.rows),
